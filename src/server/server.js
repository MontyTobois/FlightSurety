...
function submitOracleResponse(oracle, index, airline, flight, timestamp) {
 ...
 flightSuretyApp.methods
  ...
  .send({ from: oracle, gas: 450000, gasPrice: 20000000 }, (error, result) => {
   if (error) {
 ...
 if (defaultStatus == STATUS_CODE_LATE_AIRLINE) {
  flightSuretyData.methods
   .creditInsuree(flight)
   .call({ from: oracle }, (error, result) => {
    if (error) {
     console.log(error, payload);
    } else {
     console.log("Credit set for insuree");
     // console.log(payload);
     // console.log(result);
    }
   });
     ...
     flightSuretyApp.methods
      ...
      .then(() => {
       ...
       flightSuretyApp.methods
        ...
        .then((result) => {
         console.log(
          `Oracle ${a} Registered at ${accounts[a]} with [${result}] indexes.`
         );
