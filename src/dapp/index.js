...
(async () => {
 ...
 let contract = new Contract("localhost", () => {
  DOM.elid("ToboisAir").value = contract.owner;
  DOM.elid("selected-airline-name").value = "ToboisAir";
    ...
    display("", "Register new flight", [
     {
      ...
      error: error,
      value:
       "Flight code: " + result.flight + " Destination: " + result.destination
     }
    ]);
    if (!error) {
  ...
  DOM.elid("airlineDropdownOp").addEventListener("click", (e) => {
   e.preventDefault();

   DOM.elid("selected-airline-name").value = e.target.innerHTML;
   DOM.elid("selected-airline-address").value = e.target.value;
  });
 });
 ...
 DOM.elid("flights-display").addEventListener("click", async (e) => {
  let flightCode = e.target.innerHTML;
  console.log(e);
...
function addAirlineList(airlineName, hash) {
 var dropdown = DOM.elid("airlineDropdownList");
